...
def metric(prompt):
    ...
    dataset = get_eval_data_detect_ai(["CLTS: 100"], prompt)
    ...
    dataset = get_eval_data_detect_ai(["deepseek: 100"], prompt)
    pbar = tqdm.tqdm(total=len(dataset))
    for data in dataset:
        text = data
        output = generate(text['input'])['response'][0]
        ...
        tot1 += 1
        pbar.update(1)
        pbar.set_description_str(f"CLTS: acc: {acc1}/{tot1}/{null1}-{acc1 / tot1}")
    acc += acc1
    null += null1
    tot += tot1
...
def updata_prompt(save_dir, original_path):
    ...
    while True:
        now = _update(prompts['prompt'][0][0])
        print(now)
        score = metric(now)
...
if __name__ == '__main__':
    updata_prompt("/netcache/hekaiyu/project/prompt/detect_ai2.json",
                  "/netcache/hekaiyu/project/prompt/detect_ai.json")
