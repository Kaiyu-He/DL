...
if __name__ == "__main__":
    # 1. 从test.src读取前100行数据，并清理文本
    with open("test.src", "r", encoding="utf-8") as f:
        articles = [{"id": f"line_{i+1}", "article": clean_text(line.strip())}
                   for i, line in enumerate(f) if line.strip()][:100]  # 只取前100个
...
"""
        ...
        results["details"].append({
            ...
            "article_content": article[:100] + "..." if len(article) > 100 else article,
            "api_result": result,
    ...
    # 4. 保存结果到JSON文件
    output_filename = f"detection_human_results_{time.strftime('%Y%m%d_%H%M%S')}.json"
    ...
    print(f"'是'的数量: {results['statistics']['yes_count']} ({(results['statistics']['yes_count'] / results['statistics']['total_articles']) * 100:.2f}%)")
    print(f"'否'的数量: {results['statistics']['no_count']} ({(results['statistics']['no_count'] / results['statistics']['total_articles']) * 100:.2f}%)")
    print(f"未识别数量: {results['statistics']['unrecognized']} ({(results['statistics']['unrecognized'] / results['statistics']['total_articles']) * 100:.2f}%)")
